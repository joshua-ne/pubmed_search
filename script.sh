...
jobID=$(python code/assign_jobID.py)
...
python code/create_folder.py $jobID $query

python code/retrieve_ID_list.py $jobID $query
...
python code/efetch_ID_title_json.py $jobID $query

python code/efetch_detail.py $jobID $query
...
python code/concatenate.py $jobID $query

python code/concatenate_ID_title.py $jobID $query

python code/sentence_tokenization.py $jobID $query

python code/rmspace.py $jobID $query
